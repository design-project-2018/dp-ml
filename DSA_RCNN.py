...
class DSA_RCNN:
    ...
    def __init__(self, verbose=True):
        ...
        for i in range(n_frames):
            print('Creating graph for frame {}/{}'.format(i, n_frames))
    ...
    def crop_object_features(self, frame, coord_list):
        network=self.VGG_model
        obj_list = []
    ...
    def objects_from_clip(self, path, start, finish, network):
        obj_frames = []
        ctr = start
        while (ctr <= finish):
            # Read JSON file with
    ...
    def predict(self, file_path, start, finish, model_path='./model/'):
        network=self.VGG_model
...
model = DSA_RCNN()
classifications = model.predict('/home/nvidia/Downloads/output/', 1, 100)